...
export const AnimatedSpan = ({
  ...
  return (
    <motion.div
      ...
      onAnimationComplete={() => {
        // Only trigger sequence advance on the ENTER animation
...
export const Terminal = ({
  ...
  // Loop: when all items complete, wait and restart
  useEffect(() => {
    if (!sequence || !loop || totalItems === 0) return
    if (activeIndex >= totalItems && sequenceHasStarted) {
  ...
  }, [activeIndex, totalItems, sequenceHasStarted, loop, loopDelay, sequence])
  ...
  }, [sequence, activeIndex, sequenceHasStarted, loopCount])
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn(
        "relative h-full max-h-100 w-full max-w-lg rounded-xl border border-border bg-[#122423] text-gray-300",
        className
      )}
      dir="ltr"
    >
