...
function WizardOptions({ locale, questionIndex, copy }: { locale: RouteContentLocale; questionIndex: number; copy: RouteContentCopyText }) {
  ...
  return <div className="grid gap-3"><h4 className="font-black text-alt">{copy.options}</h4>{options.fields.map((field, index) => <RepeatableCard key={field.id} title={`${copy.options} ${index + 1}`} removeLabel={copy.remove} onRemove={() => options.remove(index)}><input type="hidden" {...register(`${name}.${index}.value`)} /><div className="grid gap-4 md:grid-cols-2"><FormField label={copy.title}><Input {...register(`${name}.${index}.title`)} /></FormField><FormField label={copy.description} className="md:col-span-2"><Textarea {...register(`${name}.${index}.description`)} /></FormField><FormField label={copy.budget}><Input type="number" {...register(`${name}.${index}.budget`, { valueAsNumber: true })} /></FormField><FormField label={copy.weeks}><Input type="number" {...register(`${name}.${index}.weeks`, { valueAsNumber: true })} /></FormField><FormField label={copy.multiplier}><Input type="number" step="0.01" {...register(`${name}.${index}.multiplier`, { valueAsNumber: true })} /></FormField><FormField label={copy.weekMultiplier}><Input type="number" step="0.01" {...register(`${name}.${index}.weekMultiplier`, { valueAsNumber: true })} /></FormField></div></RepeatableCard>)}<AddButton label={copy.addOption} onClick={() => options.append({ value: `option-${options.fields.length + 1}`, title: "", description: "", budget: 0, weeks: 0, multiplier: 1, weekMultiplier: 1 })} /></div>;
}
